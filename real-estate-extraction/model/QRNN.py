    ...
    def forward(self, x):
        ...
        with tf.variable_scope("QRNN/Forward"):
            ...
            if self.conv_size <= 2:
                ...
                def cond(i): return tf.less(i, c_f.get_shape()[0])

                def step(i):
                    f, z, o = c_f[i], c_z[i], c_o[i]
                    self._step(f, z, o)
                    return tf.add(i, 1)
                tf.while_loop(cond, step, [i])
        return self.h
    ...
    def __init__(self, in_size, size):
        ...
        with tf.variable_scope("QRNN/Variable/Linear"):
            ...
            self.W = tf.get_variable(
                "W", [self.in_size, self._weight_size], initializer=initializer)
    ...
    def __init__(self, in_size, size):
        ...
        with tf.variable_scope("QRNN/Variable/WithPrevious"):
            ...
            self.V = tf.get_variable(
                "V", [self.in_size, self._weight_size], initializer=initializer)
            self.b = tf.get_variable(
                "b", [self._weight_size], initializer=initializer)

    def forward(self, t):
        if self._previous is None:
            self._previous = tf.get_variable("previous", [t.get_shape(
            )[0], self.in_size], initializer=tf.random_normal_initializer())
    ...
    def __init__(self, in_size, size, conv_size):
        ...
        with tf.variable_scope("QRNN/Variable/Convolution"):
            ...
            self.conv_filter = tf.get_variable(
                "conv_filter", [conv_size, in_size, self._weight_size], initializer=initializer)
