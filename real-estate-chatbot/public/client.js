...
var Botkit = {
  ...
  renderMessage: function (message) {
    ...
    if (message.articles) {
      message.resultSliderId = 'slider' + this.result_slider_id;
      this.result_slider_id += 1;
    }
    ...
    if (message.articles) {
      var list = this.renderResultMessages(message.articles);
      var sliderContainer = document.getElementById(message.resultSliderId);  
      list.forEach(function (element) {
        sliderContainer.appendChild(element);
      })
      var slider = tns({
        container: `#${message.resultSliderId}`,
        gutter: 10,
        nav: false,
        arrowKeys: true,
        controlsContainer: '#customize-controls-' + message.resultSliderId,
      });
    }
  ...
  renderResultMessages: function(results) {
    ...
    for (var r = 0; r < results.length; r++) {
      (function (result) {
        ...
        link.innerHTML = 'Nhấp vào link để xem chi tiết';
        link.href = result._id;
        link.target = '_blank';
        // el.onclick = function () {
        //   that.quickReply(result.payload);
        // }
  ...
  boot: function (user) {
    ...
    that.on('message', function (message) {
      if (message.multiple_replies) {
        that.clearReplies();
        var list = document.createElement('table');

        var elements = [];
        var payload = 'Bỏ yêu cầu: ';
        for (var r = 0; r < message.multiple_replies.length; r++) {
          (function (reply) {

            var tr = document.createElement('tr');
            var td = document.createElement('td');
            td.innerHTML = reply.title;
            ...
            el.onclick = function () {
              payload = payload + (that.input.value ? ', ' : '') + reply.content;
              that.input.value = payload;
              el.setAttribute('style', 'background:#a795ef; color: #FFF');
            }
    ...
    that.on('message', function (message) {
      if (message.aspects_list) {
        that.clearReplies();
        
        var table = document.createElement('table');
        table.setAttribute('class', 'message_replies');

        that.message_list.appendChild(table);

        var elements = [];
        var payload = 'Bỏ yêu cầu: ';
        for (var r = 0; r < message.aspects_list.length; r++) {
          (function (reply) {
